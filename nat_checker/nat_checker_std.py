...
def get_endpoint():
    try:
        with open("/allconf/hostname.conf") as f:
...
class StunClient(object):
    ...
    stun_server_udp = ["stun.qq.com", "stun.miwifi.com"]
    MTU = 1500
    STUN_PORT = 3478
    ...
    def get_nat_type_txt(self, nat_type):
        return self.NAT_TYPE_MAP.get(nat_type, "Unknown")
...
def get_args():
    ...
    parser.add_argument(
        ...
        "--interface",
        default="ppp*",
        help="source interface for client, support wild card")
...
def main():
    ...
    if args.interface:
        ...
        if len(ifaces) == 0:
            print("No network interface found for %s" % args.interface, file=sys.stderr)
            sys.exit(1)
        for iface in ifaces:
    ...
    def run(source_ip, source_port):
        try:
            nat_type = stun_client.check_nat_type(
                source_ip, source_port)
            nat_type_txt = stun_client.get_nat_type_txt(nat_type)
        except Exception as e:
            nat_type = -1
            nat_type_txt =  str(e)[:20]
        netcard = iface_map.get(source_ip)
        if netcard:
            quene.put([netcard, nat_type, nat_type_txt])
